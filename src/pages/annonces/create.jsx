  ...
  const fileInputManager = ({ current: input }) => {
    setForm({
      ...
      detailsVehicule: {
        ...
        pieces: { ...form.detailsVehicule.pieces, [input.name]: [] },
      },
    });
  ...
  const submitForm = async () => {
    ...
    if (validator.allValid() && filesValid()) {
      ...
      try {
        ...
        createAnnonce(token, new FormData(formRef.current));
      } catch (error) {
      ...
      navigate(0);
      toast.success(
        "Annonce enregistrée avec succès. Veuillez attendre la validation d'un administrateur."
      );
    } else {
            ...
            <div className="my-2 flex flex-col gap-2">
              <div className="border flex px-2 justify-around flex-wrap">
                <div className="w-[235px] h-[106px] flex justify-center flex-shrink-0">
                  <img
                    src={
                      form.detailsVehicule.photos[0]
                        ? URL.createObjectURL(form.detailsVehicule.photos[0])
                        : "https://placehold.co/235x106.png?text=Image+de+la+voiture&font=Montserrat"
                    }
